  ...
  if (!oa_tc6_get_field(regval, CONFIG0_SYNC))
    {
      ...
      nwarn("Warning: MAC-PHY has lost config (SYNC = '0'), shutting down\n");
 ...
 *   mdd_ioctl_data_s structure.
 *
 * Input Parameters:
 *   priv - pointer to the driver-specific state structure
 *   req  - pointer to the mmd_ioctl_data_s structure with addr and dest
 *
 * Returned Value:
 *   On success OK is returned, otherwise negated errno is returned.
 *
                           ...
                           FAR struct mmd_ioctl_data_s *req)
{
  ...
  if (oa_tc6_get_mmd_base(req->mmd, &mms, &base_addr))
    {
      return -EINVAL;
    }

  regid = OA_TC6_MAKE_REGID(mms, base_addr + req->addr);
 ...
 *
 * Description:
 *   Write the MMD register from the the provided mmd_ioctl_data_s structure
 *   into the MAC-PHY.
 *
 * Input Parameters:
 *   priv - pointer to the driver-specific state structure
